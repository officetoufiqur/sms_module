...
const parameters = [
  ...
  },
  {
    ...
    required: 'Optional',
    type: 'int',
    value: 'Get batch starting with ID larger than X (if a first request is made use 0 or null).',
  }
];
...
curl --location 'https://capi.mesergo.co.il/api/v2/Contact/GetUnsubscribeList' \
...
const whitelistlink = `https://capi.mesergo.co.il/api/v2/Contact/GetUnsubscribeList`;
const advancedLink = `https://capi.mesergo.co.il/api/v2/Contact/GetUnsubscribeList?view=json`;
...
<template>
  <div class="xl:flex rounded-md space-x-10 border-b border-gray-300">
    ...
    <div class="xl:w-[50%]">
      <div class="mb-6 mt-5 border-b border-gray-300 pb-4">
        <div class="flex items-center gap-2 mb-2">
          ...
          <h1 class=" font-semibold text-gray-900">Get Unsubscribe List</h1>
        </div>
        <Murkdown :rawCode="whitelistlink" />
        <p class="mt-3 test-sm">This function Gets the list of Unsubscribed.</p>
    ...
    <div class="xl:w-[50%] bg-[#272822] h-auto rounded-md">
      <DocCode class="mt-[-.5rem]" title="Example Request" subtitle="GetUnsubscribeList" language="json"
